...
;	CMP #$09		; Is it CTRL-I (Increment Screen Colour)
;	BEQ CTRLI		; Yes, Do it

CCDONE	JMP STORKEY		; We didnt find our custom key. Pass it on. We're done!

;*****************************************************************
; Fix "syntax error" message then return
;*****************************************************************

CCFIX	LDA #0			; Try to fix "syntax error"
	STA INBUF		; put it in buffer
	JMP RUBOUT2
...
CTRLU	JSR CLEARS		; Clear to same as first bye of colour ram
	BEQ CCFIX		; We're done

;*****************************************************************
; Clear Colour Memory - !!! NOTE: currently only for 1K ram
;*****************************************************************
; CLEARI - Increment colour then clear
; CLEARS - Clears Colour Memory to Same as top left
; CLEARC - Clears Colour Memory to colour in A register

CLEARI	INC $D400		; Increment the colour
